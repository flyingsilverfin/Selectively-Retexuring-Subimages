...
"""
This is the Neural Style Transfer demo from the keras examples
encapsulated in a single class for use via other programs
"""
...
class NeuralStyleTransfer(object):
    ...
    def _deprocess_image(self, x, target_img_nrows, target_img_ncols):
        if K.image_data_format() == 'channels_first':
            x = x.reshape((3, target_img_nrows, target_img_ncols))
            x = x.transpose((1, 2, 0))
        else:
    ...
    def _total_variation_loss(self, x, img_nrows, img_ncols):
        assert K.ndim(x) == 4
        if K.image_data_format() == 'channels_first':
    ...
    def eval_loss_and_grads(self, x, f_outputs, img_nrows, img_ncols):
        if K.image_data_format() == 'channels_first':
